...
fn resolve_log_dir() -> Option<PathBuf> {
    ...
    if let Ok(home) = std::env::var("HOME") {
        candidates.push(PathBuf::from(home).join("Library/Logs/zino-storyboard-copilot"));
    }

    candidates.push(std::env::temp_dir().join("zino-storyboard-copilot/logs"));
...
fn setup_logging() {
    let env_filter = tracing_subscriber::EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| "info,zino_storyboard_copilot=debug".into());
